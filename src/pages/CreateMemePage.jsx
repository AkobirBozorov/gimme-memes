...
/** Constants for ephemeral usage (no id) **/
const LOCAL_KEY = "ephemeralMemeData";

/** Arrays for text & surface color picks **/
const TEXT_COLORS = [
  "#000000",
  "#FFFFFF",
  "#FF0000",
...
/** Common font sizes (numeric only). We’ll handle custom via +/− **/
...
function CreateMemePage() {
  ...
  // Toolbars
  const [openDropdown, setOpenDropdown] = useState(null); // Possibly used in primary
  ...
  /** Load existing or ephemeral meme on mount **/
  useEffect(() => {
    if (id) {
      loadExistingMeme(id);
    } else {
  ...
  /** If user selects an overlay, always show the secondary toolbar (fix mobile) **/
  useEffect(() => {
  ...
  function storeEphemeralData(
    ...
    if (id) return; // If editing a real meme, skip ephemeral
    const ephemeral = {
      realWidth: w,
      realHeight: h,
  ...
  function handleDeleteOverlay() {
    ...
    commitOverlays(
      displayOverlays.filter((ov) => ov.id !== selectedOverlayId)
    );
    setSelectedOverlayId(null);
  }
  ...
  /** Save Meme: merges overlays, uploads final image **/
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-gray-200 p-6 flex flex-col items-center">
      ...
      {/* Secondary Toolbar: below the preview */}
...
/** Primary Toolbar with essential actions **/
...
/** Secondary Toolbar: text editing with hidden color pickers replaced by a single displayed button each **/
    ...
    <div className="flex flex-col bg-gray-100 rounded-lg p-4 shadow space-y-4">
      <div className="flex flex-wrap justify-around items-center gap-4">
        ...
        <div className="flex flex-col items-center">
          ...
          <div className="flex items-center space-x-2">
            ...
            <select
              ...
              value={isInList ? currentFontSize : ""}
            >
              {/* Custom placeholder if not in list */}
              {!isInList && (
        ...
        <div className="flex flex-col items-center">
          ...
          {/* Single color button showing current color */}
          <button
            className="w-8 h-8 rounded-full border border-gray-300"
            style={{ backgroundColor: selectedOverlay?.textColor || "#000000" }}
            onClick={() => setShowTextColorPicker(!showTextColorPicker)}
          />
          {/* If toggled, show color swatches */}
          {showTextColorPicker && (
            <div className="absolute mt-2 p-2 bg-white border rounded shadow flex flex-wrap gap-1 z-50">
              {TEXT_COLORS.map((c) => (
                <button
                  key={c}
                  onClick={() => {
        ...
        <div className="flex flex-col items-center relative">
          ...
          {/* Single color button for surface */}
          <button
            className="w-8 h-8 rounded-full border border-gray-300 flex items-center justify-center"
            style={{
              backgroundColor: selectedOverlay?.bgColor || "transparent",
            }}
            onClick={() => setShowSurfaceColorPicker(!showSurfaceColorPicker)}
          >
